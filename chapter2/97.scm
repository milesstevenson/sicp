#lang racket
;;; Part A is finished. Part B is lacking.
;;;
...
(define (install-polynomial-package)
  ...
  (define (reduce-terms n d)
    (let* ((g (gcd-terms n d))
          (factor (exp (coeff (first-term g))
                       (+ 1 (- (max (order (first-term n))
                                    (order (first-term d)))
                               (order (first-term g)))))))
      (list (car (div-terms (mul-term-by-all-terms (make-term 0 factor) n)
                       g))
            (car (div-terms (mul-term-by-all-terms (make-term 0 factor) d)
                       g)))))
  (define (reduce-poly p1 p2)
    (if (same-variable? (variable p1) (variable p2))
        (let ((reduced-terms (reduce-terms (term-list p1) (term-list p2))))
          (list (make-poly (variable p1) (adjoin-term (car reduced-terms) (the-empty-termlist)))
                (make-poly (variable p2) (adjoin-term (cadr reduced-terms) (the-empty-termlist)))))
        (error "Polynomials don't use the same variables" 
               (list (variable p1) (variable p2)))))
